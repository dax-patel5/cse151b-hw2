...
def make_dataset(mode, data_root):
    ...
    items = []
    if mode == 'train':
        img_path = os.path.join(data_root, 'VOCdevkit', 'VOC2012', 'JPEGImages')
        mask_path = os.path.join(data_root, 'VOCdevkit', 'VOC2012', 'SegmentationClass')
        data_list = [l.strip('\n') for l in open(os.path.join(
            data_root, 'VOCdevkit', 'VOC2012', 'ImageSets', 'Segmentation', 'train.txt')).readlines()]
        for it in data_list:
        ...
        mask_path = os.path.join(data_root, 'VOCdevkit', 'VOC2012', 'SegmentationClass')
        data_list = [l.strip('\n') for l in open(os.path.join(
            data_root, 'VOCdevkit', 'VOC2012', 'ImageSets', 'Segmentation', 'trainval.txt')).readlines()]
        for it in data_list:
            item = (os.path.join(img_path, it + '.jpg'), os.path.join(mask_path, it + '.png'))
            items.append(item)
    else:
        ...
        mask_path = os.path.join(data_root, 'VOCdevkit', 'VOC2012', 'SegmentationClass')
        data_list = [l.strip('\n') for l in open(os.path.join(
            data_root, 'VOCdevkit', 'VOC2012', 'ImageSets', 'Segmentation', 'val.txt')).readlines()]
        for it in data_list:
            item = (os.path.join(img_path, it + '.jpg'), os.path.join(mask_path, it + '.png'))
            items.append(item)
...
class VOC(data.Dataset):
    ...
    def __init__(self, mode, transform=None, target_transform=None, data_root = "./data"):
